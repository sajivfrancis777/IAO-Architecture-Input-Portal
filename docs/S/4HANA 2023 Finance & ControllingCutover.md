...
| | |
|---|---|
| **Migration Path** | ICOST → SAP ECC → SAP S/4HANA 2023 |
| **Version** | 1.1 |
| **Focus Area** | Material Ledger Cutover Sequencing |
| **Base Scenario** | Mid-Year Cutover (Scenario B) |
...
2. [Cutover Scenario Matrix](#2-cutover-scenario-matrix)
3. [ICOST Migration Context](#3-icost-migration-context)
4. [End-to-End Architecture Overview](#4-end-to-end-architecture-overview)
5. [Material Ledger — What Changes in S/4HANA 2023](#5-material-ledger--what-changes-in-s4hana-2023)
6. [Material Extension — Per-Plant Requirements](#6-material-extension--per-plant-requirements)
7. [Material Ledger Cutover — The Core Cycle](#7-material-ledger-cutover--the-core-cycle)
8. [WIP Balances — Design Decision](#8-wip-balances--design-decision)
9. [Master Cutover Sequence](#9-master-cutover-sequence)
...
> ⚠️ **Critical design decision:** ICOST cost components must be mapped to SAP cost component structure before cutover. Any mismatch between ICOST cost categories and SAP cost components will cause COGS split errors from day one. This mapping should be completed and validated in the Explore phase.
>
> *(Product Cost Controlling with SAP.pdf — product-costing)*
...
## 5. Material Ledger — What Changes in S/4HANA 2023

> In S/4HANA, Material Ledger is **mandatory** for all plants. The data model has been significantly restructured.

### 5.1 Data Model Mapping (ECC → S/4HANA)
...
### 5.2 Key Simplification: Four-Step Close → Single Settlement

The four separate ECC closing steps — Single-Level Price Determination, Multilevel Price Determination, Revaluation of Consumption, WIP Revaluation — are **merged into one step called Settlement** in S/4HANA. Plan your cutover runbook around the new single-step model, not the ECC four-step model.

*(SIMPL_OP2022.pdf — product-costing)*

---

## 6. Material Extension — Per-Plant Requirements
...
### 6.1 Material Master Settings Required
...
### 6.3 Pre-Requisite Checks Before CKMSTART (Per Plant)
...
## 7. Material Ledger Cutover — The Core Cycle
...
### 7.2 What CKMSTART Does Per Plant

When run, CKMSTART:

1. Creates ML master data (CKMLHD, CKMLPR) for all existing materials in the plant
2. Translates all material prices and stock values into the ML currencies defined in OMX2/OMX3
3. Translates existing PO histories into the second and third ML currencies
4. Initializes cost component split data for existing inventory — sourced from the standard cost estimate; if no cost component split exists on the standard cost, the total is summarized into the material consumption component

*(Actual Costing with the SAP Material Ledger — product-costing)*

### 7.3 Upload Cycle: Timing and Sequence
...
## 8. WIP Balances — Design Decision

This is the most complex element of the cutover given open production orders mid-flight. Three design options are evaluated below.

### 8.1 Option 1: Settle All WIP Before Cutover (Recommended)
...
| | Pros | Cons |
|---|------|------|
| 1 | Cleanest cutover — no WIP complexity in S/4HANA opening | Operationally disruptive — production must pause before go-live |
| 2 | FI = CO reconciliation is straightforward | Settlement of partially complete orders distorts period P&L |
| 3 | No special cutover BOM/routing required | May not be feasible for long-cycle manufacturing |

### 8.2 Option 2: Load WIP as Balance Sheet Opening Entry (Pragmatic)
...
> The clearing account is reversed post go-live once the production order in S/4HANA absorbs the costs through normal goods issue and confirmation postings.
>
> *(SAP Activate methodology for S4HANA cFIN.xlsx — product-costing)*

### 8.3 Option 3: Direct Transfer via Migration Cockpit (Complex)
...
> **Option 2** (Balance Sheet entry + Cutover BOM/Routing) is the most pragmatic and auditable approach for complex manufacturing with ICOST legacy costs.

---

## 9. Master Cutover Sequence

### 9.1 Pre-Cutover (Weeks Before Go-Live)
...
### 9.2 Cutover Weekend — Day 1: Foundation
...
| D1-3 | Extract final ECC trial balance | F.01 | Reconciliation baseline |
